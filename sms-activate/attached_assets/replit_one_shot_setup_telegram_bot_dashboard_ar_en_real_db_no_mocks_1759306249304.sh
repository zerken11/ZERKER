...
cat > locales/ar/common.json <<'EOF'
{
  ...
  "ui": {"language": "اللغة", "arabic": "العربية", "english": "English", "welcome": "مرحبًا {{name}}", "prices": "الأسعار", "save": "حفظ", "admin": "لوحة المدير"},
  "bot": {
    "start": "أهلاً! أنا البوت الرسمي. اختر طريقة البدء 👇",
...
cat > locales/en/common.json <<'EOF'
{
  ...
  "ui": {"language": "Language", "arabic": "العربية", "english": "English", "welcome": "Welcome, {{name}}", "prices": "Prices", "save": "Save", "admin": "Admin"},
  "bot": {
    "start": "Hey! I’m the official bot. Choose how to start 👇",
...
<div class="card">
  <h2><%= t('ui.admin') %></h2>
  <p><small class="muted">Basic admin panel.</small></p>
  <ul>
    <li>Users count: <strong><%= stats.users %></strong></li>
    <li>WhatsApp – EG: <strong><%= prices.EG %> <%= currency %></strong></li>
    <li>WhatsApp – CA: <strong><%= prices.CA %> <%= currency %></strong></li>
  </ul>
</div>
EOF
...
if (BOT_TOKEN) {
  ...
  function buildStartKeyboard(lang){
    ...
    const row1 = [
      Markup.button.callback(`▶️ ${t('bot.btn_start_wa')}`, 'start_wa'),
    ];
    if (PUBLIC_BASE_URL) {
  ...
  bot.start((ctx) => {
    ...
    return ctx.reply(t('bot.start'), buildStartKeyboard(lang));
  });
  ...
  bot.action('admin_dashboard', (ctx)=>{
    ...
    return ctx.editMessageText(t('bot.open_admin', { url: `${PUBLIC_BASE_URL}/admin` }), buildStartKeyboard(lang));
  });
  ...
  bot.action('back_to_start', (ctx)=>{
    const lang = (getUserByTgId(String(ctx.from.id))?.lang) || START_LANG;
    const t = (k)=> i18next.getFixedT(lang)(k);
    return ctx.editMessageText(t('bot.start'), buildStartKeyboard(lang));
  });
  ...
  bot.action(/set_lang:(ar|en)/, (ctx) => {
    ...
    const t = (k)=> i18next.getFixedT(lang)(k);
    return ctx.editMessageText(t('bot.lang_set_' + (lang==='ar'?'ar':'en')), buildStartKeyboard(lang));
  });
