...
function parseLegacyKV(text) {
  ...
  const p = text.split(':');
  if (p[0]==='ACCESS_NUMBER' && p.length>=3) return { id: p[1], phone: p[2] };
  if (p[0]==='STATUS_OK') return { code: p.slice(1).join(':').trim() };
  return null;
}

class SmsActivateClient {
  constructor(apiKey, testMode=false, log=console) { this.apiKey=apiKey; this.testMode=testMode; this.log=log; }

  async getCountries() {
    if (this.testMode) return { Egypt:{id:24, eng:'Egypt'}, Canada:{id:2, eng:'Canada'} };
  ...
  async getPrices(service, country) {
    ...
    return { cost: parseFloat(svc.cost), count: parseInt(svc.count||'0',10) };
  }

  async getNumber(service, country) {
    if (this.testMode) return { id: String(Math.floor(Math.random()*1e9)), phone: '+20-123-456-7890' };
    ...
    if (data && data.activationId && data.phoneNumber) return { id: String(data.activationId), phone: data.phoneNumber };
