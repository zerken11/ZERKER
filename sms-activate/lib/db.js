...
async function init() {
  ...
  try {
    ...
    await fsp.writeFile(
      DB_PATH,
      JSON.stringify({ users: {}, activations: {}, tokens: {} }, null, 2),
    );
  }
}
...
function _enqueueWrite(data) {
  writeQueue = writeQueue.then(() =>
    fsp.writeFile(DB_PATH, JSON.stringify(data, null, 2)),
  );
  return writeQueue;
}
...
async function saveActivation(act) {
  const db = await _read();
  db.activations[act.id] = act;
  await _enqueueWrite(db);
}
async function getActivation(id) {
  const db = await _read();
  return db.activations[id] || null;
}
async function listUserActivations(uid, limit = 10) {
  const db = await _read();
  return Object.values(db.activations)
