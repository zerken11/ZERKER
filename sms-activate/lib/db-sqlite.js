...
function init() {
  ...
  try { db.prepare("ALTER TABLE users ADD COLUMN credits_cents INTEGER DEFAULT 0").run(); } catch(e) {}

  db.prepare(`CREATE TABLE IF NOT EXISTS activations (
    id TEXT PRIMARY KEY,
  ...
  db.prepare(`CREATE INDEX IF NOT EXISTS idx_users_username_local ON users(username_local)`).run();
  db.prepare(`CREATE INDEX IF NOT EXISTS idx_activations_user_id ON activations(user_id)`).run();
...
function migrateFromJSON() {
  ...
  try {
    ...
    if (jsonData.users) {
      const insertUser = db.prepare(`INSERT OR IGNORE INTO users (id, balance, first_name, username, username_local, pass_hash, lang, created_at) 
    ...
    if (jsonData.activations) {
      const insertAct = db.prepare(`INSERT OR IGNORE INTO activations (id, user_id, phone, service, country, price, code, status, created_at)
    ...
    if (jsonData.tokens) {
      const insertToken = db.prepare(`INSERT OR IGNORE INTO tokens (token, uid, created_at, expires_at) VALUES (@token, @uid, @created_at, @expires_at)`);
...
function upsertUser(user) {
  ...
  if (existing) {
    ...
    if (user.balance !== undefined) { updates.push('balance = @balance'); params.balance = user.balance; }
    if (user.first_name !== undefined) { updates.push('first_name = @first_name'); params.first_name = user.first_name; }
    if (user.firstName !== undefined) { updates.push('first_name = @first_name'); params.first_name = user.firstName; }
    if (user.username !== undefined) { updates.push('username = @username'); params.username = user.username; }
    if (user.username_local !== undefined) { updates.push('username_local = @username_local'); params.username_local = user.username_local; }
    if (user.passHash !== undefined) { updates.push('pass_hash = @pass_hash'); params.pass_hash = user.passHash; }
    if (user.lang !== undefined) { updates.push('lang = @lang'); params.lang = user.lang; }
    
    if (updates.length > 0) {
      db.prepare(`UPDATE users SET ${updates.join(', ')} WHERE id = @id`).run(params);
    }
  } else {
    db.prepare(`INSERT INTO users (id, balance, first_name, username, username_local, pass_hash, lang, created_at)
      VALUES (@id, @balance, @first_name, @username, @username_local, @pass_hash, @lang, @created_at)`).run({
      id,
      balance: user.balance || 0,
      first_name: user.first_name || user.firstName || null,
      username: user.username || null,
...
function getUserByUsername(username) {
  const row = db.prepare('SELECT * FROM users WHERE LOWER(username_local) = LOWER(?)').get(String(username));
  if (!row) return null;
  return {
    id: row.id,
    balance: row.balance,
...
function saveActivation(act) {
  db.prepare(`INSERT OR REPLACE INTO activations (id, user_id, phone, service, country, price, code, status, created_at)
    VALUES (@id, @user_id, @phone, @service, @country, @price, @code, @status, @created_at)`).run({
    id: String(act.id),
    user_id: String(act.userId),
    phone: act.phone || null,
    service: act.service || null,
...
function getActivation(id) {
  const row = db.prepare('SELECT * FROM activations WHERE id = ?').get(String(id));
  if (!row) return null;
  return {
    id: row.id,
    userId: row.user_id,
...
function listUserActivations(uid, limit = 10) {
  const rows = db.prepare('SELECT * FROM activations WHERE user_id = ? ORDER BY created_at DESC LIMIT ?').all(String(uid), limit);
  return rows.map(row => ({
    id: row.id,
    userId: row.user_id,
    phone: row.phone,
...
function createToken(token, uid, ttlMs = 1000 * 60 * 60 * 24 * 7) {
  db.prepare(`INSERT OR REPLACE INTO tokens (token, uid, created_at, expires_at) VALUES (?, ?, ?, ?)`).run(
    token,
    String(uid),
    Date.now(),
    Date.now() + ttlMs
...
function updatePrice(service, country, amount) {
  db.prepare(`UPDATE prices SET amount = ? WHERE service = ? AND country = ?`).run(amount, service, country);
}
...
function addCreditCents(targetTgId, deltaCents, adminTgId, note) {
  ...
  db.prepare('UPDATE users SET credits_cents = ? WHERE id = ?').run(newBal, String(targetTgId));
  db.prepare('INSERT INTO credit_tx(created_at, admin_tg_id, user_tg_id, delta_cents, note) VALUES (?,?,?,?,?)')
    .run(new Date().toISOString(), String(adminTgId), String(targetTgId), deltaCents, note || null);
